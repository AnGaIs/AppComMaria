...
import com.umservodemaria.appcommaria.adapter.ArticleAdapter;
            ...
            public void onResponse(Call<List<Article>> call, Response<List<Article>> response) {
                if(response.isSuccessful()){
                    ...
                    for (int i = 0; i < response.body().size(); i++) {
                        ...
                        if (i==0){
                            if (getActivity().getApplicationContext().getResources().getString(R.string.article_ads_enabled).equals("true")) {
                                articleList.add(new Article().setViewType(2));
                            }
                        }
                        next_article_id = response.body().get(i).getId();
                        loading = true;
                    }
                    if (getActivity().getApplicationContext().getResources().getString(R.string.article_ads_enabled).equals("true")) {
                        articleList.add(new Article().setViewType(2));
